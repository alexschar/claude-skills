...
- Use independent verification when the same actor should not both create and
  approve high-impact output. For visual/feel acceptance against a reference, use
  `/fact-check` §2's blind-comparison leg (blindness is same-medium-pairs only;
  cross-medium gets a non-author judge + a named, Alex-blessed judging set).
